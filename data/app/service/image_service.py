...
def update_image(img:str,not_nb_time:float,nb_time:float):
    time = not_nb_time
    if not_nb_time == -1:
        time = None
    numba_time = nb_time
    if numba_time == -1:
        numba_time= None
    image : Image  = image_repository.update_image(engine=engine,img=img,temps=time,temps_numba=numba_time)
    return ImageResponse(img=image.image,numba_time=image.elapsed_time_numba,time=image.elapsed_time)
